...
if __name__ == '__main__':
    ...
    base_tweet = ""
    while base_tweet != "-1":
        base_tweet = raw_input("Enter text:")
        end_time_str = raw_input("Enter time window upper bound (YYYYMMDD)")
        end_time = datetime.datetime(int(end_time_str[0:4]),int(end_time_str[4:6]),int(end_time_str[6:8])).strftime('%s')
        start_time_str = raw_input("Enter time window lower bound (YYYYMMDD)")
